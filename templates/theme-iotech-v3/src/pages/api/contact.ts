...
export const POST: APIRoute = async ({ request }) => {
	...
	try {
		...
		if (ct.includes("application/json")) {
			...
			lastname   = String(f.get("name")      ?? "").trim();
	...
	if (email.length   > MAX_LEN.email)   return Response.json({ error: "Email trop long" },       { status: 400 });
	if (phone.length   > MAX_LEN.phone)   return Response.json({ error: "Téléphone trop long" },   { status: 400 });
	if (message.length > MAX_LEN.message) return Response.json({ error: "Message trop long" },     { status: 400 });
